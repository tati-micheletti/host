isGDALInstalled <- function () {
  findGdalInstallationPaths <- function (search_path = NULL, rescan = FALSE, ignore.full_scan = TRUE, 
                                         verbose = FALSE){
    # Based on raptr package function: findGdalInstallationPaths
    ...
    gdal_drivers <- function(path, verbose = FALSE) {
      ...
      for (i in seq_along(drivers_raw)) {
        ...
        driver_names <- gsub("^ ", "", sapply(drivers, function(x) {
          x[2]
        }))
        driver_codes_perm <- strsplit(sapply(drivers, function(x) {
          x[1]
        }), "\\(")
    ...
    gdal_version <- function(path, newerThan = NULL, verbose = FALSE) {
      ...
      if (sum(res) != length(result)) 
        message("Probably broken install of gdal at '", paste0(path[which(res != 
                                                                            1)], collapse = "' and '"), "'")
      result <- result[res == 1]
      date <- version <- vector(mode = "list", length = length(result))
      for (i in seq_along(result)) {
        ingd <- strsplit(result[[i]], ",")[[1]]
        version[[i]] <- gsub(ingd[1], pattern = "GDAL ", 
                             replacement = "")
        ind <- grep(ingd, pattern = "releas")
        date[[i]] <- as.character(as.Date(gsub(ingd[ind], 
                                               pattern = " released ", replacement = ""), format = "%Y/%m/%d"))
      }
      if (!is.null(newerThan)) {
        ...
        if (!inherits(test, "try-error")) {
          ...
          version <- gsub(tolower(version), pattern = "[a-z]", 
                          replacement = "")
      ...
      result <- as.data.frame(cbind(path = path[res == 1], 
                                    version = version, date = date), stringsAsFactors = FALSE)
      return(result)
    }
    ...
    gdal_check_validity <- function(path) {
      checkValidity <- sapply(path, function(x) {
        cmd <- normalizePath(list.files(path = x, pattern = "^gdalinfo$|^gdalinfo\\.exe$", 
                                        full.names = TRUE))
        if (length(cmd) == 0) {
          return(FALSE)
        }
          ...
          cmd <- paste0("\"", cmd, "\"", " --version")
          validity <- length(try(gdal <- system(cmd, intern = TRUE), 
                                 silent = TRUE))
        ...
        if (!ignore.common && length(path) == 0) {
          ...
          if (.Platform$OS == "unix") {
            common_locations <- c("/usr/bin", "/usr/local/bin", 
                                  "/Library/Frameworks/GDAL.framework", "/opt/local/bin")
          }
    ...
    gdal_installation <- function(return_versions = TRUE, return_drivers = TRUE, 
      ...
      if (is.null(path)) 
        return(NULL)
      gdal_installation_results <- lapply(path, function(x, 
                                                         return_drivers, return_python_utilities, return_versions) {
        result <- list(path = x)
        if (return_versions) {
        ...
        return(result)
      }, return_drivers = return_drivers, return_python_utilities = return_python_utilities, 
      ...
      if (sort_most_current) {
        ...
        gdal_installation_results <- gdal_installation_results[order(as.Date(unlist(versions)), 
                                                                     decreasing = TRUE)]
      }
    ...
    gdal_installation_out <- gdal_installation(search_path = search_path, 
                                               rescan = rescan, ignore.full_scan = ignore.full_scan, 
                                               verbose = verbose)
